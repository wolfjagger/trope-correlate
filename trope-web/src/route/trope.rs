...
pub fn TropeRelationship() -> Html {
  ...
  html! {
    <>
      ...
      <h1>{ "Trope relationships" }</h1>

      <h2>{ "Plot" }</h2>

      <Button onmousedown={ onclick_line }>{ "Line plot" }</Button>
      <Button onmousedown={ onclick_para }>{ "Parabola plot" }</Button>
      <Button onmousedown={ onclick_dir_graph }>{ "Important directed graph" }</Button>
      <Button onmousedown={ onclick_und_graph }>{ "More important undirected graph" }</Button>
