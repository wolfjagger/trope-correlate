...
fn read_html_file(file_name: path::PathBuf, encrypted: bool) -> Result<Html, Box<dyn std::error::Error>> {
  ...
  // Input file
  let html_content = if encrypted {

    let mut enc_content = fs::File::open(file_name)?;
    ...
    BrotliDecompress(&mut enc_content, &mut html_writer)?;

    String::from_utf8(html_writer.into_inner()?)?

  } else {

    let mut html_content = String::new();
    fs::File::open(file_name)?.read_to_string(&mut html_content)?;
    html_content
